...
for celltype in celltype_labels:
    ...
    pt.compute_selection_metrics(
        adata=adata,
        min_k=archetypes_to_test[0],
        max_k=archetypes_to_test[-1],
        n_jobs=20,
    )
    ...
    pt.compute_bootstrap_variance(
        adata=adata, n_archetypes_list=archetypes_to_test, n_bootstrap=10
    )
    ...
    p = pt.plot_bootstrap_2D(adata, result_filters={"n_archetypes": number_of_archetypes_dict[celltype]})
        ...
        for seed in pbar:
            ...
            proc = mp_ctx.Process(
                ...
                args=(
                    ...
                    optim_dict,
                    seed,
                    number_of_archetypes_dict[celltype],
                ),
            )
            proc.start()
...
result_df["description"] = [
    celltype + " | " + str(n_samples) + " | " + str(n_arch) + " | " + str(n_dim)
    for celltype, n_samples, n_arch, n_dim in zip(
        result_df["celltype"],
        result_df["n_samples"],
        result_df["n_archetypes"],
        result_df["n_dimensions"],
    )
]
result_df["rss_norm"] = result_df["rss"] / (
    result_df["n_samples"] * result_df["n_dimensions"]
)
result_df["key"] = [
    init + "__" + optim
    for init, optim in zip(result_df["init_alg"], result_df["optim_alg"])
]
...
p = (
    pn.ggplot(agg_df)
    + pn.geom_point(
        pn.aes(x="time__mean", y="rss__mean", color="optim_alg", shape="init_alg"),
        size=4,
    )
    + pn.geom_errorbar(
        pn.aes(x="time__mean", ymin="rss__ymin", ymax="rss__ymax", color="optim_alg"),
        width=0,
    )
    + pn.geom_errorbarh(
        pn.aes(y="rss__mean", xmin="time__xmin", xmax="time__xmax", color="optim_alg"),
        height=0,
    )
    ...
    + pn.scale_color_manual(
        values={"projected_gradients": "green", "frank_wolfe": "blue"}
    )
)
...
p = (
    pn.ggplot(agg_df)
    + pn.geom_point(
        pn.aes(x="time__mean", y="rss_norm__mean", color="optim_alg", shape="init_alg"),
        size=4,
    )
    + pn.geom_errorbar(
        pn.aes(
    ...
    + pn.geom_errorbarh(
        pn.aes(
            y="rss_norm__mean", xmin="time__xmin", xmax="time__xmax", color="optim_alg"
        ),
        height=0,
    )
    + pn.facet_wrap(facets="description", ncol=3, scales="free")
    + pn.labs(
        x="Time (s)",
        y="Normalized Residual Sum of Squares (RSS)",
        color="Optimization Algorithm",
        shape="Initialization Algorithm",
    )
    + pn.theme_bw()
...
for celltype in celltype_list:
    ...
    df["optim"] = pd.Categorical(
        df["optim"],
        categories=[
            b + "__" + a for a in pt.const.OPTIM_ALGS for b in pt.const.INIT_ALGS
        ],
    )
    ...
    p = (
        pn.ggplot(df)
        + pn.geom_hline(
            yintercept=min(df["rss_trace"]), color="black", alpha=0.5, size=1
        )
        + pn.geom_line(
            pn.aes(x="iter", y="rss_trace", color="seed"), alpha=1.0, size=0.5
        )
        + pn.scale_y_log10()
        + pn.facet_wrap("optim")
        + pn.theme_bw()
