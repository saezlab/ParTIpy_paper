...
figure_dir = Path(FIGURE_PATH) / "ms_xenium_coreset_v1"
figure_dir.mkdir(exist_ok=True, parents=True)

output_dir = Path(OUTPUT_PATH) / "ms_xenium_coreset_v1"
    ...
    for coreset_fraction in coreset_fraction_arr:
        ...
        for seed in pbar:
            ...
            if coreset_fraction == 1.0:
                reference_archetypes_pos_dict[seed] = bench_results["Z"]
            ...
            for seed_key, ref_arch in reference_archetypes_pos_dict.items():
                query_arch = bench_results["Z"].copy()
                ...
                euclidean_distance_per_matched_arch = np.linalg.norm(
                    ref_arch[_ref_idx, :] - query_arch[query_idx, :], axis=1
                )
            ...
            result_dict = {
                ...
                "rss": bench_results["RSS_full"],
                "varexpl": bench_results["varexpl"],
...
# result_df = pd.read_csv(Path(OUTPUT_PATH) / "ms_xenium_coreset" / "results.csv")
...
for yvar in ["min_l2_distance_to_ref", "mean_gex_corr"]:
    ncols = 4
    n_celltypes = result_df["celltype"].nunique()
    nrows = int(np.ceil(n_celltypes / ncols))
