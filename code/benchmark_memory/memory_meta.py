...
#######
df_0 = pd.read_csv(
    Path("output") / "k562_memory_bench_only_pca" / "k562_memory_results.csv"
)
df_0["dataset"] = "k562"

df_1 = pd.read_csv(
    Path("output") / "hct116_memory_bench_only_pca" / "hct116_memory_results.csv"
)
df_1["dataset"] = "hct116"

df_2 = pd.read_csv(
    Path("output") / "hek293t_memory_bench_only_pca" / "hek293t_memory_results.csv"
)
df_2["dataset"] = "hek293t"

df = pd.concat([df_0, df_1, df_2])
df["coreset_fraction"] = pd.Categorical(df["coreset_fraction"])
df.to_csv(output_dir / "memory_meta_benchmark_summary_only_pca.csv", index=False)
...
p1.save(figure_dir / "memory_usage_memory_meta_only_pca.pdf", dpi=300, width=8, height=6)
p1.save(figure_dir / "memory_usage_memory_meta_only_pca.png", dpi=300, width=8, height=6)

p2 = p1 + pn.scale_y_log10()
p2.save(figure_dir / "memory_usage_memory_meta_logy_only_pca.pdf", dpi=300, width=8, height=6)
p2.save(figure_dir / "memory_usage_memory_meta_logy_only_pca.png", dpi=300, width=8, height=6)
...
p3.save(figure_dir / "computation_time_memory_meta_only_pca.pdf", dpi=300, width=8, height=6)
p3.save(figure_dir / "computation_time_memory_meta_only_pca.png", dpi=300, width=8, height=6)

p4 = p3 + pn.scale_y_log10()
p4.save(
    figure_dir / "computation_time_memory_meta_logy_only_pca.pdf", dpi=300, width=8, height=6
)
p4.save(
    figure_dir / "computation_time_memory_meta_logy_only_pca.png", dpi=300, width=8, height=6
)
